...
const CartModal: React.FC<CartModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-opacity-40 flex items-end justify-center">
      <div className="bg-white w-full max-w-md h-[92vh] rounded-t-2xl overflow-y-auto p-4 border-t-[1px] border-gray-300">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-bold">سبد خرید</h2>
          <button onClick={onClose} className="text-sm text-red-500 cursor-pointer hover:bg-gray-100 w-10 h-10 hover:rounded-xl">
            بستن
          </button>
        </div>
              ...
              <li key={item.id} className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-600">{item.originalPrice} تومان</p>
                  </div>
                </div>
                <button
